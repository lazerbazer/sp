...
-- ========== FAKE COUNT SYSTEM ==========
local fakeItemCounts = {}

local function ReapplyFakeItems()
    for itemType, items in pairs(fakeItemCounts) do
        if ProfileData[itemType] and ProfileData[itemType].Owned then
            for itemKey, count in pairs(items) do
                if count and count > 0 then
                    ProfileData[itemType].Owned[itemKey] = count
                else
                    ProfileData[itemType].Owned[itemKey] = nil
                end
            end
        end
    end
...
local ManualBlockedKeys = {
	["niksscythe"] = "manually blocked (Nik's Scythe)",
}
	...
	for key, data in pairs(source) do
		if type(data) == "table"
		   and (data.ItemType == "Knife" or data.ItemType == "Gun") then
			...
			local flatName = _blockKey(name)
			local blockReason = ManualBlockedKeys[flatName] or WeaponBlockReason(name, rarity, data)
			if blockReason then
...
local function PurgeBlockedFromInventory()
	...
	pcall(function()
		...
		for k, v in pairs(owned) do
			...
			if itemKey then
				...
				if type(data) ~= "table" then
					...
					reason = ManualBlockedKeys[_blockKey(displayName)] or WeaponBlockReason(displayName, data.Rarity or "Common", data)
				end
		...
		for _, item in ipairs(kill) do
			owned[item.k] = nil
			if fakeItemCounts.Weapons and fakeItemCounts.Weapons[item.k] then
				fakeItemCounts.Weapons[item.k] = nil
			end
			removed = removed + 1
		end
...
local function IsValidItem(itemName, itemType)
	itemType = itemType or "Weapons"
	return Sync[itemType] and Sync[itemType][itemName] ~= nil
end

local SaveFile = "AXION_Inventory_Backup.txt"
local function SaveInventory()
	local success, result = pcall(function()
		local json = game:GetService("HttpService"):JSONEncode(fakeItemCounts)
		writefile(SaveFile, json)
		print("[mm2run] Inventory saved to: " .. SaveFile .. " (" .. #json .. " bytes)")
		return true
	end)
	if not success then
		warn("[mm2run] Save failed: " .. tostring(result))
		return false
	end
	return true
end
...
local function LoadInventory()
    if not isfile(SaveFile) then
        print("[mm2run] No save file found at: " .. SaveFile)
        return false
    end
    ...
    if not success then
        print("[mm2run] Failed to read save file: " .. tostring(json))
        return false
    end
    ...
    if not success then
        print("[mm2run] Failed to parse save file (corrupt).")
        return false
    end

    if type(data) ~= "table" then
        print("[mm2run] Invalid save data (not a table).")
        return false
    end

    fakeItemCounts = {}  -- reset
    local loadedCount = 0
    for itemType, items in pairs(data) do
        if type(items) == "table" then
            fakeItemCounts[itemType] = {}
            for itemKey, count in pairs(items) do
                if count and type(count) == "number" and count > 0 then
                    fakeItemCounts[itemType][itemKey] = count
                    loadedCount = loadedCount + 1
                end
            end
        end
    end
    print("[mm2run] Inventory loaded from: " .. SaveFile .. " (" .. loadedCount .. " items restored)")
    return true
end

task.wait(1)
pcall(LoadInventory)
pcall(ReapplyFakeItems)
...
local function SpawnItem(ItemName, Amount, ItemType)
	...
	pcall(function()
		if not fakeItemCounts[ItemType] then fakeItemCounts[ItemType] = {} end
		local current = fakeItemCounts[ItemType][ItemName] or 0
		fakeItemCounts[ItemType][ItemName] = current + Amount
		ReapplyFakeItems()
		game.ReplicatedStorage.Remotes.Inventory.InventoryDataChanged:Fire()
	end)
end
...
local function RemoveItem(ItemName, Amount, ItemType)
	pcall(function()
		if fakeItemCounts[ItemType] and fakeItemCounts[ItemType][ItemName] then
			local newCount = fakeItemCounts[ItemType][ItemName] - Amount
			if newCount <= 0 then
				fakeItemCounts[ItemType][ItemName] = nil
			else
				fakeItemCounts[ItemType][ItemName] = newCount
			end
			ReapplyFakeItems()
			game.ReplicatedStorage.Remotes.Inventory.InventoryDataChanged:Fire()
		end
...
titleLabel.Text = "giancastin on discord"
...
-- ================================================================
local function CreatePopOutWindow(tabName, contentFrame)
	local popOutGui = Instance.new("ScreenGui")
	popOutGui.Name = "PopOut_" .. tabName
	popOutGui.ResetOnSpawn = false
	popOutGui.DisplayOrder = 999999999
	popOutGui.Parent = game:GetService("CoreGui")
	...
	closeBtn.Position = UDim2.new(1, -26, 0.5, 0)
	...
	-- Content container (clones the tab's ScrollingFrame)
	local contentClone = contentFrame:Clone()
	contentClone.Size = UDim2.new(1, 0, 1, -28)
	contentClone.Position = UDim2.new(0, 0, 0, 28)
	contentClone.BackgroundTransparency = 0
	contentClone.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
	contentClone.BorderSizePixel = 0
	contentClone.Parent = window

	-- Fix any anchors or positions inside the clone (since parent changed)
	for _, child in ipairs(contentClone:GetDescendants()) do
		if child:IsA("GuiObject") then
			-- Reset position if it was anchored to the old parent's edges
			-- But keep relative offsets; we just need to ensure scrolling works.
		end
	end
	...
	local resizeStart, resizePos, resizeSize
	resizeHandle.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			resizeStart = input.Position
			resizePos = window.Position
			resizeSize = window.Size
		end
	end)

	UserInputService.InputChanged:Connect(function(input)
		if not resizeStart then return end
		if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
		local delta = input.Position - resizeStart
		local newW = math.max(200, resizeSize.X.Offset + delta.X)
		local newH = math.max(150, resizeSize.Y.Offset + delta.Y)
...
for i, tabName in ipairs(tabs) do
	...
	-- Right-click to pop out
	tabButton.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton2 then
			CreatePopOutWindow(tabName, tabFrame)
		end
	end)
end
...
-- ================================================================
--  CUSTOM PARTNER ROW WITH RANDOM BUTTON (word-based + numbers)
-- ================================================================
local partnerRow = Instance.new("Frame")
partnerRow.Size = UDim2.new(1, 0, 0, 35)
partnerRow.BackgroundTransparency = 1
partnerRow.Parent = controlFrame

local partnerLayout = Instance.new("UIListLayout")
partnerLayout.FillDirection = Enum.FillDirection.Vertical
partnerLayout.SortOrder = Enum.SortOrder.LayoutOrder
partnerLayout.Padding = UDim.new(0, 1)
partnerLayout.Parent = partnerRow

local heading = Instance.new("TextLabel")
heading.Size = UDim2.new(1, 0, 0, 15)
heading.BackgroundTransparency = 1
heading.Text = "Partner user:"
heading.Font = Enum.Font.SourceSansSemibold
heading.TextSize = 12
heading.TextColor3 = Color3.fromRGB(180, 180, 180)
heading.TextXAlignment = Enum.TextXAlignment.Left
heading.Parent = partnerRow

local boxContainer = Instance.new("Frame")
boxContainer.Size = UDim2.new(1, 0, 0, 25)
boxContainer.BackgroundTransparency = 1
boxContainer.Parent = partnerRow
...
box.BackgroundColor3 = Color3.fromRGB(40, 40, 50)
box.BackgroundTransparency = 0.2
box.Text = TradeTable.Player2.Player
box.Font = Enum.Font.SourceSans
box.TextSize = 14
box.TextColor3 = Color3.fromRGB(255, 255, 255)
box.ClearTextOnFocus = false
...
-- 🎲 Random button (generates real-sounding names with occasional numbers)
local randomBtn = Instance.new("TextButton")
randomBtn.Size = UDim2.new(0, 28, 0, 22)
randomBtn.Position = UDim2.new(1, -32, 0.5, -11)
...
randomBtn.Parent = boxContainer

local randCorner = Instance.new("UICorner")
randCorner.CornerRadius = UDim.new(0, 4)
randCorner.Parent = randomBtn

-- Word lists for generating realistic names
local prefixes = {
	"Cool", "Night", "Shadow", "Dark", "Light", "Fire", "Ice", "Storm",
	"Thunder", "Dragon", "Phoenix", "Wolf", "Fox", "Hawk", "Eagle",
	"Venom", "Crystal", "Neon", "Cyber", "Omega", "Alpha", "Zeta",
	"Lunar", "Solar", "Cosmic", "Nova", "Galaxy", "Apex", "Raven",
	"Frost", "Flame", "Steel", "Silver", "Golden", "Midnight", "Dawn",
	"Sky", "Ocean", "Star", "Moon", "Sun", "Blaze", "Ghost", "Spirit"
}
local suffixes = {
	"Wolf", "Fox", "Hawk", "Eagle", "Strike", "Gamer", "Lord", "King",
	"Queen", "Master", "Slayer", "Hunter", "Fang", "Claw", "Tail",
	"Blade", "Shield", "Storm", "Fire", "Ice", "Thunder", "Dragon",
	"Phoenix", "Venom", "Crystal", "Neon", "Cyber", "Omega", "Alpha",
	"Zeta", "Lunar", "Solar", "Cosmic", "Nova", "Galaxy", "Apex",
	"Raven", "Frost", "Flame", "Steel", "Silver", "Golden", "Midnight",
	"Dawn", "Sky", "Ocean", "Star", "Moon", "Sun", "Blaze", "Ghost",
	"Spirit", "Storm", "Chill", "Burn", "Crush", "Dash", "Fury", "Rage"
}
local standalones = {
	"Shadow", "Night", "Dragon", "Phoenix", "Venom", "Crystal",
	"Neon", "Cyber", "Omega", "Alpha", "Zeta", "Lunar", "Solar",
	"Cosmic", "Nova", "Galaxy", "Apex", "Raven", "Frost", "Flame",
	"Steel", "Silver", "Golden", "Midnight", "Dawn", "Sky", "Ocean",
	"Star", "Moon", "Sun", "Blaze", "Ghost", "Spirit"
}

randomBtn.MouseButton1Click:Connect(function()
	local name
	if math.random() < 0.3 then  -- 30% chance of a standalone word
		name = standalones[math.random(1, #standalones)]
	else
	...
	-- Add a number about 50% of the time (1-99, or 1-999 for extra variety)
	if math.random() < 0.5 then
		if math.random() < 0.3 then  -- 30% of the time, use 3 digits
			name = name .. tostring(math.random(100, 999))
		else
			name = name .. tostring(math.random(1, 99))
		end
	end

	-- Cap length to avoid ridiculously long names
	if #name > 16 then
		name = name:sub(1, 16)
	end
...
box.FocusLost:Connect(function()
	TradeTable.Player2.Player = box.Text
	box.Text = TradeTable.Player2.Player
end)

PartnerUserBox = box  -- keep the global reference
...
local function SilentBlockPlayer(Selected)
	...
	repeat
		...
		modal = SilentBlockServices.CoreGui:FindFirstChild("BlockingModalScreen", true)
	until modal

	pcall(function()
		modal.BlockingModalContainer.BlockingModalContainerWrapper.BlockingModal.BackgroundTransparency = 1
		modal.BlockingModalContainer.BlockingModalContainerWrapper.BackgroundTransparency = 1
		modal.BlockingModalContainer.BackgroundTransparency = 1
		modal.BlockingModalContainer.BlockingModalContainerWrapper.BlockingModal.AlertModal.Position =
			UDim2.new(0.00800000038, -110, 0.5, 0)
	end)

	local blockButton = nil
	pcall(function()
		blockButton = modal.BlockingModalContainer.BlockingModalContainerWrapper.BlockingModal.AlertModal.AlertContents.Footer.Buttons["3"]
	end)

	if blockButton then
		print("[mm2run/block] Block button found at " .. blockButton:GetFullName())
		SilentBlockServices.GuiService.SelectedObject = blockButton
		task.wait()
		if SilentBlockServices.GuiService.SelectedObject == blockButton then
			SilentBlockServices.VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.Return, false, game)
			SilentBlockServices.VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.Return, false, game)
			task.wait()
		end
		SilentBlockServices.GuiService.SelectedObject = nil
	else
...
CreateSpace(controlFrame)
CreateButton(controlFrame, "Refresh Inventory (Clockwork)", function()
	pcall(function()
		local valid = IsValidItem("Clockwork", "Weapons")
		if valid then
			SpawnItem("Clockwork", 1, "Weapons")
		else
			SpawnItem("TheSeer", 1, "Weapons")
		end
		game.ReplicatedStorage.Remotes.Inventory.InventoryDataChanged:Fire()
		if Config.in_trade then
			pcall(functions.UpdateTrade)
		end
	end)
end)
...
CreateSpace(controlFrame)
CreateButton(controlFrame, "Save Inventory", function()
	SaveInventory()
end)

-- ========== LOAD INVENTORY BUTTON ==========
CreateSpace(controlFrame)
CreateButton(controlFrame, "Load Inventory", function()
    local success = LoadInventory()
    if success then
        ReapplyFakeItems()
        game.ReplicatedStorage.Remotes.Inventory.InventoryDataChanged:Fire()
        if Config.in_trade then
        ...
        print("[mm2run] Inventory loaded and reapplied.")
    end
end)

-- ========== AUTO-BLOCK ZERO VALUE ==========
local autoBlockEnabled = false
local autoBlockTask = nil
local blockedPlayers = {}

local function AutoBlockZeroValue()
    while autoBlockEnabled do
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= Players.LocalPlayer and not blockedPlayers[player.Name] then
                local inv = FetchPlayerInventory(player)
                if inv then
                    local total, _ = CalculateInventoryValue(inv, player.Name)
                    print("[AutoBlock] " .. player.Name .. " value = " .. total)
                    if total <= 0 then
                        SilentBlockPlayer(player)
...
local autoBlockToggle, getAutoBlockState = CreateToggleButton(controlFrame, "Auto Block 0 Value", function(state)
    autoBlockEnabled = state
    if autoBlockEnabled then
        if not autoBlockTask or autoBlockTask:GetStatus() == Enum.TaskStatus.Dead then
            autoBlockTask = task.spawn(AutoBlockZeroValue)
        end
...
-- ========== SERVER HOP BUTTONS ==========
CreateSpace(controlFrame)
CreateButton(controlFrame, "Server Hop", function()
    pcall(function()
...
local function GetServerList()
    ...
    local requestFunc = (syn and syn.request) or (http and http.request) or (fluxus and fluxus.request) or http_request or request
    if requestFunc then
        local success, result = pcall(function()
            return requestFunc({
                Url = url,
...
CreateSpace(controlFrame)
CreateButton(controlFrame, "Server Hop Low", function()
    task.spawn(function()
        local servers = GetServerList()
        if not servers then
            warn("[ServerHop Low] Failed to fetch server list.")
            return
        end
        local currentJobId = game.JobId
        for _, server in ipairs(servers) do
            if server.id ~= currentJobId and server.playing == 1 then
                print("[ServerHop Low] Joining server with 1 player: " .. server.id)
                pcall(function()
        ...
        print("[ServerHop Low] No servers with 1 player found.")
    end)
end)
...
-- ================================================================
--  UPDATED WEAPON LIST – added new items
-- ================================================================
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
	...
	for _, info in ipairs(weaponButtons) do
		if query == "" or string.find(string.lower(info.button.Text), query) then
			info.button.Visible = true
		else
			info.button.Visible = false
		end
	end
	weaponScrollFrame.CanvasPosition = Vector2.new(0, 0)
end)
...
local spawnHighTierBtn = CreateButton(spawnerFrame, "Spawn High Tier (Tradable)", function()
	local count, total = 0, 0
	for _, entry in ipairs(WeaponCatalog) do
		if SpawnerHighTierSet[entry.rarity] then
	...
	print(("[mm2run/spawner] High-tier bulk spawn: %d weapon types, %d items total"):format(count, total))
end)
...
CreateButton(spawnerFrame, "Purge untradables from inventory", function()
	...
	spawnerStatusLabel.Text = ("Removed %d untradable weapon(s) -- see console"):format(removed)
	spawnerStatusLabel.TextColor3 = (removed > 0)
		and Color3.fromRGB(120, 255, 160)
		or Color3.fromRGB(255, 200, 120)
end)
...
do
    local raw = {
        -- ============================================================
        -- CHROMA WEAPONS (Supreme Values)
        -- ============================================================
        ["Chroma Traveler's Gun"]  = 220000,
        ["Chroma Evergun"]         = 74000,
        ["Chroma Evergreen"]       = 60000,
        ["Chroma Bauble"]          = 38000,
        ["C. Constellation"]       = 36000,
        ["C. Vampire's Gun"]       = 39000,
        ["Chroma Alienbeam"]       = 35500,
        ["Chroma Raygun"]          = 15000,
        ...
        ["Chroma Treat"]           = 6750,
        ["C. Heart Wand"]          = 7000,
        ["Chroma Snowstorm"]       = 4500,
        ["Chroma Watergun"]        = 3350,
        ["Chroma Sweet"]           = 7750,
        ["Chroma Ornament"]        = 3250,
        ["Chroma Beachy"]          = 1650,
        ["Chroma Sands"]           = 1800,
        ["Chroma Icecream"]        = 1750,

        -- ============================================================
        -- ANCIENT WEAPONS (Supreme Values)
        -- ============================================================
        ["Corrupt"]                = 400,
        ["Gingerscope"]            = 17750,
        ["Traveler's Axe"]         = 8100,
        ...
        -- ============================================================
        -- GODLY WEAPONS (Supreme Values – August 2026)
        -- ============================================================
        ["Traveler's Gun"]         = 5600,
        ["Evergun"]                = 3450,
        ["Constellation"]          = 2700,
        ...
        -- ============================================================
        -- ADDITIONAL GODLIES (including new items)
        -- ============================================================
        ["Ornament"]               = 38,
        ["Green Luger"]            = 25,
        ["Amerilaser"]             = 23,
