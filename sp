...
-- Manual block for specific items (e.g. Nik's Scythe)
...
local function PurgeBlockedFromInventory()
	...
	pcall(function()
		...
		for _, item in ipairs(kill) do
			owned[item.k] = nil
			-- Also remove from fake counts so ReapplyFakeItems doesn't re-add it
...
-- Save inventory to file (uses executor's writefile)
...
-- Load saved inventory automatically on script start
task.wait(1)
pcall(LoadInventory)
...
function StartTrade()
	...
	ReapplyFakeItems() -- Ensure fake counts are in ProfileData before generating inventory
...
for i, tabName in ipairs(tabs) do
	...
	-- Changed from Frame to ScrollingFrame for scrollable tabs
...
-- ========== NEW SILENT BLOCK (from second script) ==========
...
local function SilentBlockPlayer(Selected)
	...
	-- Hide the modal visuals (optional)
	pcall(function()
	...
	-- Find and click the Block button using VirtualInputManager
...
-- Improved HTTP fetch for server list (with fallback)
local function GetServerList()
    local url = "https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?limit=100"
    local body = nil

    -- Try executor request functions
...
-- Search box for weapons list (from second script)
...
-- Connect search box to filter weapon buttons
...
-- EvoPrefixes / _isEvoWeapon / _isTradable now live with the untradable
-- filter above the WeaponCatalog build, so blocked weapons never get this far.
...
local spawnHighTierBtn = CreateButton(spawnerFrame, "Spawn High Tier (Tradable)", function()
	-- WeaponCatalog is already stripped of untradables by the filter above
...
do
    local raw = {
        ...
        -- All values verified from https://supremevalues.com/mm2/godlies
        -- ============================================================
        ["Traveler's Gun"]         = 5600,   -- Verified
        ["Evergun"]                = 3450,   -- Verified
        ["Constellation"]          = 2700,   -- Verified
        ["Evergreen"]              = 2550,   -- Verified
        ["Turkey"]                 = 2450,   -- Verified
        ["Alienbeam"]              = 2350,   -- Verified
        ["Vampire's Gun"]          = 1950,   -- Verified
        ["Raygun"]                 = 1900,   -- Verified
        ["Darkshot"]               = 1800,   -- Verified
        ["Darksword"]              = 1775,   -- Verified
        ["Blossom"]                = 1370,   -- Verified
        ["Sakura"]                 = 1360,   -- Verified
        ["Sunrise"]                = 1125,   -- Verified
        ["Snowcannon"]             = 850,    -- Verified
        ["Bauble"]                 = 825,    -- Verified
        ["Sunset"]                 = 625,    -- Verified
        ["Soul"]                   = 615,    -- Verified
        ["Spirit"]                 = 605,    -- Verified
        ["Rainbow Gun"]            = 420,    -- Verified
        ["Flora"]                  = 410,    -- Verified
        ["Rainbow"]                = 410,    -- Verified
        ["Bloom"]                  = 400,    -- Verified
        ["Heart Wand"]             = 340,    -- Verified
        ["Xenoknife"]              = 295,    -- Verified
        ["Xenoshot"]               = 295,    -- Verified
        ["Ocean"]                  = 285,    -- Verified (was 265)
        ["Waves"]                  = 280,    -- Verified (was 220)
        ["Flowerwood Gun"]         = 265,    -- Verified
        ["Blizzard"]               = 260,    -- Verified (was 260, already correct)
        ["Flowerwood"]             = 260,    -- Verified
        ["Snowstorm"]              = 260,    -- Verified (was 270)
        ["Snow Dagger"]            = 250,    -- Verified
        ["Watergun"]               = 250,    -- Verified
        ["Treat"]                  = 155,    -- Verified
        ["Sweet"]                  = 150,    -- Verified
        ["Borealis"]               = 145,    -- Verified (was 145, already correct)
        ["Australis"]              = 140,    -- Verified (was 140, already correct)
        ["Beachy"]                 = 130,    -- Verified
        ["Sands"]                  = 130,    -- Verified (was 150)
        ["Bat"]                    = 120,    -- Verified
        ["Candy"]                  = 80,     -- Verified (was 80, already correct)
        ["Heartblade"]             = 65,     -- Verified
        ["Icecream"]               = 120,    -- Verified

        -- ============================================================
        -- ADDITIONAL GODLIES (Supreme Values)
        -- ============================================================
        ["Ornament"]               = 38,
        ["Green Luger"]            = 25,
        ["Amerilaser"]             = 23,
        ...
        ["Spider"]                 = 12,
        ["Tides"]                  = 12,
        -- All items below value 10 have been REMOVED
    }

    -- Normalize names and build the lookup table
